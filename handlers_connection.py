"""Connection lifecycle for Hotjar Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from hotjar_client import HotjarClient
...
@chat.function(
    "connect_hotjar",
    "Connect Hotjar account via credentials.",
    action_type="write",
    chain_callable=True,
    event="hotjar-connector.connect_hotjar",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_hotjar(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Hotjar Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Hotjar Account",
...
@chat.function(
    "list_connections",
    "List connected Hotjar accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_hotjar",
    "Disconnect Hotjar account.",
    action_type="write",
    chain_callable=True,
    event="hotjar-connector.disconnect_hotjar",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_hotjar(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
